...
def save_mail(mail: List[Dict[str, str]]) -> None:
    ...
    Args: mail (list): list of dictionaries representing the mail entries to save
    Returns: None
    """
...
def add_mail(mail_entry: Dict[str, str]) -> str:
    ...
    Args: mail_entry (dict): dictionary representing the mail entry to add
...
def delete_mail(mail_id: str) -> bool:
    ...
    Args: mail_id (str): id of the mail entry to delete
    Returns: bool: True if the mail was deleted, False otherwise
    """
    mail = load_mail()
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    ...
    Args: mail_id (str): id of the mail entry to get
    Returns: dict: dictionary representing the mail entry if it exists, None otherwise
    """
    mail = load_mail()
    for entry in mail:
        if entry['id'] == mail_id:
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    ...
    Returns: list: list of dictionaries representing the mail entries
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def add_mail_route():
    ...
    Returns: str: id of the new mail entry
    """
...
def delete_mail_route(mail_id: str):
    ...
    Args: mail_id (str): The id of the mail entry to delete
    Returns: bool: True if the mail was deleted, False otherwise
    """
    success = delete_mail(mail_id)
    if success:
        res = jsonify({'message': f'Mail {mail_id} deleted successfully'})
        res.status_code = 200
    else:
        res = jsonify({'error': f'Mail {mail_id} not found'})
        res.status_code = 404
    return res
...
def get_mail_route(mail_id: str):
    ...
    Args: mail_id (str): The id of the mail entry to get
    Returns: dict: A dictionary representing the mail entry if it exists, None otherwise
    """
...
def get_sent_route(sender: str):
    """
    Summary: Gets all mail entries sent by a specific sender from the json file
    Args: sender (str): sender whose sent mail to retrieve
    Returns: list: list of dictionaries representing the mail entries sent by the sender
    """
